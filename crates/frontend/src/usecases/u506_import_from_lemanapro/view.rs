...
pub fn ImportWidget() -> impl IntoView {
    ...
    const SESSION_KEY: &str = "u506_session_id";
    const PROGRESS_KEY: &str = "u506_progress";
    ...
    Effect::new(move || {
        spawn_local(async move {
            match api::get_connections().await {
                Ok(conns) => {
                    // Фильтруем только LemanaPro подключения
                    let lemanapro_conns: Vec<_> = conns
                        .into_iter()
                        .filter(|c| c.marketplace_id == "mp-lemana")
                        .collect();
                    if let Some(first) = lemanapro_conns.first() {
                        set_selected_connection.set(first.to_string_id());
                    }
                    set_connections.set(lemanapro_conns);
                }
                Err(e) => {
        ...
        <div class="import-widget" style="padding: 20px; border: 1px solid #ccc; border-radius: 8px; max-width: 800px; margin: 20px auto; max-height: 80vh; overflow-y: auto;">
                    ...
                    if connections.get().is_empty() {
                        view! {
                            <div style="padding: 10px; background: #fff3cd; border: 1px solid #ffc107; border-radius: 4px; color: #856404;">
                                "Нет доступных подключений LemanaPro. Создайте подключение в разделе 'Подключения МП'."
                            </div>
                        }.into_any()
                    } else {
                        view! {
            ...
            <div style="margin: 20px 0;">
                ...
                <div style="margin-top: 5px; font-size: 12px; color: #666;">
                    "API: GET /b2bintegration-products/v1/products"
                </div>
                <div style="margin-top: 5px; font-size: 12px; color: #ff9800;">
                    "⚠ Примечание: Цены товаров не загружаются (требуется отдельный API запрос)"
                </div>
            </div>
            ...
            <div style="margin: 20px 0;">
                ...
                <div style="margin-top: 5px; font-size: 12px; color: #666;">{"Период используется для фильтрации (если поддерживается API)."}</div>
            </div>
            ...
            <div style="margin: 20px 0;">
                <button
                    ...
                    prop:disabled=move || is_loading.get() || session_id.get().is_some() || connections.get().is_empty()
                >
                    ...
                    if let Some(_conn) = connections.get().iter().find(|c| c.to_string_id() == conn_id) {
                        view! {
                            <div style="margin: 20px 0; padding: 10px; background: #e3f2fd; border-radius: 4px; border: 1px solid #90caf9;">
                                ...
                                <div style="font-family: monospace; font-size: 12px; color: #555;">
                                    "Base URL: https://api.lemanapro.ru"
                                </div>
                            </div>
                        }.into_any()
                    } else {
