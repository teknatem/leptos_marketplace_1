...
use contracts::projections::p909_mp_order_line_turnovers::dto::MpOrderLineTurnoverDto;
...
#[component]
pub fn MpOrderLineTurnoverDetail(
    id: String,
    #[prop(into)] on_close: Callback<()>,
) -> impl IntoView {
    let tabs_store = use_context::<AppGlobalContext>().expect("AppGlobalContext not found");
    let (data, set_data) = signal::<Option<MpOrderLineTurnoverDto>>(None);
    ...
    Effect::new(move |_| {
        ...
        spawn_local(async move {
            match fetch_detail(&current_id).await {
                ...
                Err(err) => {
                    log!("Failed to fetch p909 detail: {}", err);
    ...
    view! {
        <PageFrame page_id="p909_mp_order_line_turnovers--detail" category="detail" class="page--wide">
            <div class="page__header">
                <div class="page__header-left">
                    {icon("database")}
                    <h1 class="page__title">"P909 Order Line Turnover"</h1>
                </div>
                            ...
                            <section class="proj-detail__hero">
                                <div class="proj-detail__hero-top">
                                    <div class="proj-detail__hero-title">
                                        <div class="proj-detail__eyebrow">"Projection P909"</div>
                                ...
                                <div class="proj-detail__hero-meta">
                                    ...
                                    <div class="proj-detail__chip">
                                        <span class="proj-detail__chip-label">"Event"</span>
                                        <span>{item.event_kind.as_str().to_string()}</span>
                                    </div>
                                    <div class="proj-detail__chip">
                                        <span class="proj-detail__chip-label">"Link"</span>
                                        <span>{item.link_status.clone()}</span>
                                    </div>
                                </div>
                            ...
                            <div class="proj-detail__sections">
                                <section class="proj-detail__section">
                                    ...
                                    <div class="proj-detail__kv-grid">
                                        <KvField label="Order Key" value=item.order_key.clone() mono=true />
                                        <KvField label="Line Key" value=item.line_key.clone() mono=true />
                                        <KvField label="Line Event Key" value=item.line_event_key.clone() mono=true />
                                        ...
                                        />
                                        <KvField
                                            label="Marketplace Product Ref"
                                            value=item.marketplace_product_ref.clone().unwrap_or_else(|| "-".to_string())
                                            mono=true
                                        />
                                    </div>
                                </section>
...
async fn fetch_detail(id: &str) -> Result<MpOrderLineTurnoverDto, String> {
    ...
    let url = format!("/api/p909/order-line-turnovers/{}", urlencoding::encode(id));
    let request =
