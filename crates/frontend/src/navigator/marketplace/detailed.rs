//! Detailed view of the marketplace navigator: cards with annotations,
//! entity-type badges, marketplace chips, and technical tab key.
...
fn DetailedCard(link: &'static NavLink, ctx: AppGlobalContext) -> impl IntoView {
    ...
    view! {
        ...
        >
            <div class="navigator-mp__card-head">
                <span class="navigator-mp__card-icon">{icon(link.icon)}</span>
                <span>{link.label}</span>
            </div>
            <div class="navigator-mp__card-annotation">{link.annotation}</div>
        </a>
    }
}
