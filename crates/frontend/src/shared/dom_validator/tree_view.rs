...
pub fn TreeView(node: DomNode) -> impl IntoView {
    ...
    // Разворачивать только первые 8 уровней (0-7), table, panel-left и right-panel всегда свернуты
    let initially_collapsed = is_table || is_panel_left || is_right_panel || node.depth >= 8;
                    ...
                    view! {
                        <span class="dom-tree-node__classes">
                            {node.classes.iter().map(|cls| {
                                ...
                                let is_special = cls == "panel-left" || cls == "right-panel" || cls.starts_with("page");
                                view! {
                                    <span
                    ...
                    view! {
                        <span class="dom-tree-node__data-attrs">
                            {node.data_attributes.iter().map(|(key, value)| {
                                view! {
                                    <span class="dom-tree-node__data-attr">
                                        {format!("{}=\"{}\"", key, value)}
                                    </span>
                                }
                            }).collect_view()}
