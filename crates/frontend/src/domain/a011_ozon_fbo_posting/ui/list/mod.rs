...
pub fn OzonFboPostingList() -> impl IntoView {
    ...
    view! {
        <div>
            <h2>"OZON FBO Posting (A011)"</h2>
            <button on:click=move |_| { load_postings(); } style="padding: 8px 16px; margin: 20px 0; background: #4CAF50; color: white; border: none; border-radius: 4px; cursor: pointer;">
                "Load Postings"
            </button>
            {move || if loading.get() {
