...
pub struct WbSearchAnalyticsListDto {
    pub id: String,
...
const TABLE_ID: &str = "a040-wb-search-analytics-table";
const COLUMN_WIDTHS_KEY: &str = "a040_wb_search_analytics_column_widths";

#[component]
pub fn WbSearchAnalyticsList() -> impl IntoView {
    let tabs_store =
    ...
    let open_detail = move |id: String, document_date: String| {
        let title = format!("Поиск WB {}", document_date);
        tabs_store.open_tab(
            &format!("a040_wb_search_analytics_daily_details_{}", id),
            &title,
        );
    };
    ...
    let load_items = move || {
        spawn_local(async move {
            ...
            let mut url = format!(
                "{}/api/a040/wb-search-analytics/list?date_from={}&date_to={}&limit={}&offset={}&sort_by={}&sort_desc={}&_ts={}",
                api_base(),
                date_from_val,
                date_to_val,
                page_size,
                ...
                .await
            {
                Ok(response) if response.ok() => match response.json::<PaginatedResponse>().await {
                    Ok(paginated) => {
                        let parsed: Vec<WbSearchAnalyticsListDto> = paginated
                            .items
                            .into_iter()
                            .filter_map(|v| {
                                Some(WbSearchAnalyticsListDto {
    ...
    // Импорт снимка поисковой аналитики через общий механизм u504.
    let start_import = move || {
        let connection_id = state
            .with_untracked(|s| s.selected_connection_id.clone())
            .filter(|value| !value.is_empty());
        let Some(connection_id) = connection_id else {
            set_error.set(Some("Для сбора выберите кабинет в фильтрах".to_string()));
            return;
        };
    ...
    view! {
        <PageFrame page_id="a040_wb_search_analytics_daily--list" category="list">
            <div class="page__header">
                <div class="page__header-left">
                    <h1 class="page__title">"Поисковая аналитика WB"</h1>
                    ...
                    <Table attr:id=TABLE_ID attr:style="width: 100%; min-width: 1100px;">
                        <TableHeader>
                            <TableRow>
                                ...
                                <TableHeaderCell resizable=false min_width=110.0 class="resizable">
                                    <div class="table__sortable-header" style="cursor: pointer; text-align: right; justify-content: flex-end;" on:click=move |_| toggle_sort("total_impressions")>
                                        "Показы (нет в API WB)"
                                        <span class=move || state.with(|s| get_sort_class(&s.sort_field, "total_impressions"))>
                                            {move || get_sort_indicator(&state.with(|s| s.sort_field.clone()), "total_impressions", state.with(|s| s.sort_ascending))}
                                        </span>
                                    </div>
                                </TableHeaderCell>

                                <TableHeaderCell resizable=false min_width=110.0 class="resizable">
                                    <div class="table__sortable-header" style="cursor: pointer; text-align: right; justify-content: flex-end;" on:click=move |_| toggle_sort("total_open_card")>
                                        "Переходы"
                                        <span class=move || state.with(|s| get_sort_class(&s.sort_field, "total_open_card"))>
                                            {move || get_sort_indicator(&state.with(|s| s.sort_field.clone()), "total_open_card", state.with(|s| s.sort_ascending))}
                                        </span>
                                    </div>
                                </TableHeaderCell>
