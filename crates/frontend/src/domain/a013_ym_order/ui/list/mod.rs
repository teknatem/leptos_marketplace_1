...
pub fn YmOrderList() -> impl IntoView {
    ...
    view! {
        <div>
            <h2>"Yandex Market Orders (A013)"</h2>
            <button on:click=move |_| { load_orders(); } style="padding: 8px 16px; margin: 20px 0; background: #4CAF50; color: white; border: none; border-radius: 4px; cursor: pointer;">
                "Load Orders"
            </button>
            {move || if loading.get() {
