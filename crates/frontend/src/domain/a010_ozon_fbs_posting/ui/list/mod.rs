...
impl Sortable for OzonFbsPostingDto {
    fn compare_by_field(&self, other: &Self, field: &str) -> Ordering {
        match field {
            ...
            "delivered_at" => match (&self.delivered_at, &other.delivered_at) {
...
const TABLE_ID: &str = "a010-ozon-fbs-posting-table";
const COLUMN_WIDTHS_KEY: &str = "a010_ozon_fbs_posting_column_widths";
...
pub fn OzonFbsPostingList() -> impl IntoView {
    ...
    let all_rows: RwSignal<Vec<OzonFbsPostingDto>> = RwSignal::new(Vec::new());
    ...
    let refresh_view = move || {
        ...
        let mut filtered: Vec<OzonFbsPostingDto> = source
            .into_iter()
            .filter(|item| {
                if !date_from.is_empty() || !date_to.is_empty() {
                    if let Some(ref d) = item.delivered_at {
                        let date_part = d.split('T').next().unwrap_or(d.as_str());
                        if !date_from.is_empty() && date_part < date_from.as_str() { return false; }
                        if !date_to.is_empty() && date_part > date_to.as_str() { return false; }
                    } else {
                        return false;
                    }
                }
                                ...
                                match serde_json::from_str::<Vec<serde_json::Value>>(&text) {
                                    Ok(data) => {
                                        let items: Vec<OzonFbsPostingDto> = data
                                            ...
                                            .collect();
                                        log!("Loaded {} FBS postings", items.len());
                                        all_rows.set(items);
    ...
    let post_batch = move |post: bool| {
        ...
        spawn_local(async move {
            ...
            for (index, id) in ids.iter().enumerate() {
                ...
                let url = format!("{}/api/a010/ozon-fbs-posting/{}/{}", api_base(), id, action);
        ...
        <PageFrame page_id="a010_ozon_fbs_posting--list" category="list">
            <div class="page__header">
                <div class="page__header-left">
                    <h1 class="page__title">"Постинги FBS (OZON)"</h1>
                            ...
                            <Flex gap=FlexGap::Small align=FlexAlign::End>
                                <div style="min-width: 420px;">
                                    <DateRangePicker
                                        ...
                                        label="Период (дата доставки):".to_string()
                                    />
                                </div>
                        ...
                        <TableHeader>
                            <TableRow>
                                <TableHeaderCheckbox
                                    ...
                                    get_id=Callback::new(|row: OzonFbsPostingDto| row.id.clone())
                                    on_change=Callback::new(toggle_all)
                                />

                                <TableHeaderCell resizable=false min_width=130.0 class="resizable">
                                    <div class="table__sortable-header" style="cursor: pointer;" on:click=move |_| toggle_sort("delivered_at")>
                                        "Дата доставки"
                                        <span class=move || state.with(|s| get_sort_class(&s.sort_field, "delivered_at"))>
                                            {move || get_sort_indicator(&state.with(|s| s.sort_field.clone()), "delivered_at", state.with(|s| s.sort_ascending))}
                                        </span>
                                    </div>
                                </TableHeaderCell>
                        ...
                        <TableBody>
                            <For
                                ...
                                children=move |item| {
                                    ...
                                    let delivered_str = item.delivered_at.as_deref().map(format_date).unwrap_or_else(|| "—".to_string());
