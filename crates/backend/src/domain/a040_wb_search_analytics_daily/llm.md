...
**`show_count`** = показы (только show_count; переходы/корзина остаются за a036, чтобы не
задваивать). На чтении `SUM(COALESCE(show_count,0))` складывает показы (a040) и переходы (a036) —
воронка становится полной: **показы → переходы → корзина → заказ → выкуп**. Диагностика воронки в
u508 «Пересбор воронки за период» показывает показы автоматически после появления a040.
