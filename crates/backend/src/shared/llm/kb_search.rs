...
const STOPWORDS: &[&str] = &[
    ...
    // английский
    "the", "of", "and", "a", "to", "in", "is", "it", "for", "on", "as", "with", "at", "by", "an",
    "be", "this", "that", "or", "from", "are", "was", "were", "has", "have", "had", "not", "but",
];
        ...
        for doc in docs {
            ...
            for (term, weight) in tf {
                postings
                    .entry(term)
                    .or_default()
                    .push(Posting { doc: idx, tf: weight });
            }
...
mod tests {
    ...
    fn tag_boost_stays_commensurate_with_text_across_corpus_sizes() {
        ...
        let ratio_for = |filler: usize| {
            let mut docs = vec![
                doc("by_text", "А", &["прочее"], "конверсия корзины в заказ падает"),
                doc("by_tag", "Б", &["воронка"], "совершенно посторонний текст"),
            ];
            for i in 0..filler {
                docs.push(doc(&format!("f{i}"), "Ф", &["иное"], "нейтральный наполнитель склад логистика"));
            }
    ...
    fn bm25_ranks_specific_over_incidental() {
        let f = Fixture::new(vec![
            doc("buyout", "Лаг выкупа", &["выкуп"], "Выкуп приходит позже заказа, выкуп догоняет к концу месяца."),
            doc("misc", "Разное", &["прочее"], "Здесь один раз упомянут выкуп и много другого текста про склады и логистику."),
            doc("far", "Комиссии", &["комиссии"], "Комиссия зависит от категории товара."),
        ]);
    ...
    fn tag_hit_boosts_but_does_not_filter() {
        let f = Fixture::new(vec![
            ...
            doc("untagged", "Другое", &["прочее"], "Конверсия корзины в заказ — ключевая метрика воронки."),
        ]);
    ...
    fn deprecated_excluded_and_draft_discounted() {
        let mut draft = doc("draft", "Воронка черновик", &["воронка"], "Текст про воронку продаж.");
        draft.status = KbStatus::Draft;
        let mut dep = doc("dep", "Воронка старая", &["воронка"], "Текст про воронку продаж.");
        dep.status = KbStatus::Deprecated;
        let active = doc("active", "Воронка", &["воронка"], "Текст про воронку продаж.");
        let f = Fixture::new(vec![draft, dep, active]);

        let hits = f.run(text_query("воронка продаж"));
        assert!(!hits.iter().any(|h| h.id == "dep"), "deprecated не исключён");
        let a = hits.iter().find(|h| h.id == "active").unwrap().score;
        let d = hits.iter().find(|h| h.id == "draft").unwrap().score;
        assert!(a > d, "черновик должен быть понижен относительно активной статьи");
    ...
    fn graph_expansion_pulls_neighbour_once_and_marks_via() {
        let mut seed = doc("seed", "Лаг выкупа", &["выкуп"], "Выкуп догоняет заказ с задержкой.");
        seed.related = vec!["neighbour".into()];
        let neighbour = doc("neighbour", "Особенности WB", &["wildberries"], "Совсем другой текст.");
        let f = Fixture::new(vec![seed, neighbour]);

        let hits = f.run(text_query("выкуп"));
        let n = hits.iter().filter(|h| h.id == "neighbour").collect::<Vec<_>>();
    ...
    fn bidirectional_related_edges() {
        ...
        let parent = doc("parent", "Воронка продаж", &["воронка"], "Каноническая модель воронки.");
        let mut child = doc("child", "Диагностика", &["диагностика"], "Посторонний текст.");
    ...
    fn quality_never_manufactures_relevance() {
        let mut star = doc("star", "Комиссии", &["комиссии"], "Про комиссии маркетплейса.");
        star.stars = Some(5);
        let relevant = doc("relevant", "Выкуп", &["выкуп"], "Выкуп и лаг выкупа по когорте.");
    ...
    fn result_cap_and_relative_floor() {
        ...
        assert_eq!(outcome.total_matched, 20, "total_matched считается до отсечки");
    }

    #[test]
    fn limit_is_clamped_to_max() {
        let f = Fixture::new((0..30).map(|i| doc(&format!("d{i}"), "Воронка", &["воронка"], "воронка")).collect());
        let hits = f.run(Query {
            text: "воронка".into(),
            limit: 999,
            ..Query::default()
