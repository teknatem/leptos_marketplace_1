...
static METADATA: TaskMetadata = TaskMetadata {
    ...
    constraints: &[
        ...
        "WB отвергает интервал длиннее 31 дня, поэтому период режется на календарные месяцы: \
         число запросов = месяцы × чанки кампаний",
    ...
    config_fields: &[
        ...
        TaskConfigField {
            ...
            hint: "Максимальный диапазон за один запуск при догоняющей загрузке. \
                   Период режется на календарные месяцы автоматически (лимит WB — 31 день на запрос), \
                   поэтому крупные значения увеличивают длительность запуска, а не ломают загрузку",
            field_type: TaskConfigFieldType::Integer,
            required: false,
