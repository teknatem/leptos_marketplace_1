...
static METADATA: TaskMetadata = TaskMetadata {
    ...
    description: "Загружает поставки FBS из Wildberries и привязывает их к заказам. \
        ...
        стикеры батчами до 1000 шт. -> синхронизация income_id в a015_wb_orders. \
        Данные сохраняются в a029_wb_supply.",
    external_apis: &[ExternalApiInfo {
        name: "WB Seller API (Marketplace)",
        base_url: "https://marketplace-api.wildberries.ru/",
        rate_limit_desc: "300 запросов/мин; отдельный батч на стикеры (до 1000 шт.)",
    }],
    constraints: &[
        "Требует API-токена WB с правами на FBS-поставки",
        "Стикеры запрашиваются только для поставок в статусе ожидания сборки",
        "Синхронизирует income_id в a015_wb_orders - задействует два агрегата, но операция неразрывна",
