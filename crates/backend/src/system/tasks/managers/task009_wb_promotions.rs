...
static METADATA: TaskMetadata = TaskMetadata {
    ...
    description: "Загружает промо-календарь из Wildberries Calendar API. Последовательность \
        ...
        Данные сохраняются в a020_wb_promotion.",
    external_apis: &[ExternalApiInfo {
        name: "WB Promotions API",
        base_url: "https://promotion-api.wildberries.ru/",
        rate_limit_desc: "300 запросов/мин; последовательная загрузка деталей по каждой акции",
    }],
    constraints: &[
        "Требует API-токена WB с правами на Promotions API",
        "Загрузка деталей и номенклатур выполняется батчами для каждой акции",
...
pub struct Task009WbPromotionsManager {
    executor: Arc<ImportExecutor>,
}

impl Task009WbPromotionsManager {
...
#[async_trait]
impl TaskManager for Task009WbPromotionsManager {
    ...
    async fn run(
        ...
        logger.write_log(
            session_id,
            &format!("task009 WB Promotions: {date_from} → {date_to}"),
        )?;

        let req = ImportRequest {
            connection_id: cfg.connection_id,
            target_aggregates: vec!["a020_wb_promotion".to_string()],
            date_from,
            date_to,
        ...
        logger.write_log(session_id, "task009: WB Promotions completed")?;
