...
- **`marketing`** (верх воронки, источники `a036`/`a026`) — дневной агрегат
  ...
  a036), `cart_count`, `wishlist_count`, `funnel_order_count`, `funnel_order_sum`. Платные
  переходы и корзина хранятся отдельно в `paid_open_count`/`paid_cart_count`. «Всего показов»
  не хранится и доступно только при наличии обеих компонент free+paid; иначе это `N/A`.
