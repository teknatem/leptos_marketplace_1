...
pub async fn from_ozon_transactions(document: &OzonTransactions, document_id: &str) -> Result<Vec<Model>> {
    ...
    if posting_fbs.is_none() && posting_fbo.is_none() {
        ...
        for item in &document.items {
            ...
            let marketplace_product_ref = crate::domain::a007_marketplace_product::service::find_or_create_for_sale(
            ...
            let nomenclature_ref = if let Some(product) = crate::domain::a007_marketplace_product::service::get_by_id(marketplace_product_ref).await? {
    ...
    if let Some(fbs_doc) = posting_fbs {
        // FBS документ
        let total_amount: f64 = fbs_doc.lines.iter().map(|l| l.amount_line.unwrap_or(0.0)).sum();
        ...
        // FBO документ
        let total_amount: f64 = fbo_doc.lines.iter().map(|l| l.amount_line.unwrap_or(0.0)).sum();
